...
class StoryRename(Resource):
    ...
    def patch(self):
        ...
        parser.add_argument(
            "old_key",
            required=True,
            help="Key of file to rename - old_key - cannot be blank",
        )
        parser.add_argument(
            "old_bucket",
            required=True,
            help="Bucket of key to rename - old_bucket - cannot be blank",
        )
        parser.add_argument(
            "new_key",
            required=True,
            help="New key for file - new_key - cannot be blank",
        )
        parser.add_argument(
            "new_bucket",
            required=True,
            help="Bucket to move to - new_bucket - cannot be blank",
        )
        ...
        if not self.s3_client.check_bucket(args.old_bucket):
            return abort(
                404, description="Old bucket {} not found.".format(args.old_bucket)
            )
        ...
        copy_source = {"Bucket": args.old_bucket, "Key": args.old_key}
        self.s3_client.s3.copy_object(
